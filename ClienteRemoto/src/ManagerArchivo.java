...
public class ManagerArchivo {
    ...
    private void procesarCadena(ArrayList<String> lista) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            while (k < c.length) {
                if (e) {
                    ...
                    if (etiqueta.toLowerCase().equals("servidorVotos")
                            || etiqueta.toLowerCase().equals("ServidorRegistro")) {
                        e = false;
                    }
                } else {
                    if (v) {
        ...
        try {
            Main.serverRegistro= map.get("ServidorRegistro").toString();
            Main.serverVotos = map.get("servidorVotos").toString();
        } catch (Exception e) {
